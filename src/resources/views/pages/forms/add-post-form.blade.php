<!-- Add Post Form -->
        <form id="postContent" action="{{ route('post') }}" method="POST" enctype="multipart/form-data" class="mb-0">
            @csrf <div class="d-dashboard__create-post"><label for="large-input" class="text-lg font-semibold">Share
                    your
                    mind with us</label>
                ...
                <div class="d-dashboard__imageicon">
                    <input type="file" id="image-upload" name="images[]" multiple accept="image/*"
                        style="display: none;">@include('svg.image-upload') <button id="shareButton"
                        class="d-dashboard__post-btn">Post</button>
                </div>
            </div>
        </form>
