    ...
    <body>
        ...
        <aside class="d-dashboard__sidebar sticky top-negative">
            <div class="flex flex-col items-start gap-2 bg-gray-50 m-1 p-2 border-2 border-mygray rounded-lg">
                <ul class="flex flex-col gap-2 border-2 border-mygray rounded-lg p-2 w-full h-full bg-beige"
                    style="line-height: 1.2;">
                    <li>{{ auth()->user()->profile->first_name }}
                        {{ auth()->user()->profile->middle_name ? auth()->user()->profile->middle_name[0] . '.' : '' }}
                        {{ auth()->user()->profile->last_name }}
                    </li>
                    <li>{{ auth()->user()->email }}</li>
                    <li>Born in {{ auth()->user()->profile->birth_date }}</li>
                    ...
                    <li class="flex justify-center items-center bg-mygray w-full h-full rounded-lg text-white mt-2">
                        <a href="/profile/edit" class="block p-2">Edit Profile</a>
                    </li>
                </ul>
            </div>
            <ul class="d-dashboard__side-nav">
                ...
                <li class="d-dashboard__side-list"><a href="{{ route('logout') }}" aria-label="Logout Account"
                        class="transition-transform hover:scale-150 hover:text-red-900">Logout</a></li>
            </ul>
        </aside>
    </body>
